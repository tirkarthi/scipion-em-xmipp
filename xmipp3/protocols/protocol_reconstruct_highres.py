...
class XmippProtReconstructHighRes(ProtRefine3D, HelicalFinder):
    ...
    def _defineParams(self, form):
        ...
        form.addParam('alignmentMethod', EnumParam, label='Image alignment',
                      choices=['Global', 'Local', 'Automatic', 'Stochastic', 'Non-alignment'],
    ...
    def insertIteration(self, iteration):
        if self.alignmentMethod == self.GLOBAL_ALIGNMENT or \
                ...
                (self.alignmentMethod == self.AUTOMATIC_ALIGNMENT and iteration <= 3) or \
                self.alignmentMethod == self.NON_ALIGNMENT:
    ...
    def globalAssignment(self, iteration):
        ...
        if self.alignmentMethod == self.GLOBAL_ALIGNMENT or self.alignmentMethod == self.AUTOMATIC_ALIGNMENT or \
                self.alignmentMethod == self.STOCHASTIC_ALIGNMENT or self.alignmentMethod == self.NON_ALIGNMENT:
                ...
                for subset in perturbationList:
                    ...
                    for j in range(1, numberGroups + 1):
                        ...
                        if not exists(fnAnglesGroup):
                            ...
                            if self.alignmentMethod != self.NON_ALIGNMENT:
                                ...
                                args = '-i %s -e sort -o %s ' % (fnGroup, fnAnglesGroup)
                                self.runJob("xmipp_metadata_utilities", args, numberOfMpi=1)
                                #copyFile(fnGroup, fnAnglesGroup)
